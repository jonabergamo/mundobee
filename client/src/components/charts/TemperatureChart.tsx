...
type TemperatureChartProps = {
  ...
  isLoading?: boolean;
  error?: any;
  internalTempKey: string;
  externalTempKey: string;
...
export default function TemperatureChart({
        ...
        yaxis: [
          ...
          },
          {
            ...
            label: {
              ...
              text: minIdeal && maxIdeal && "Temperatura Ideal",
            },
          },
          {
            y: minIdeal,
  ...
  useEffect(() => {
    if (data && data.length > 0) {
      ...
      const internalTemp = data.map(
        (item: Metric) => item[internalTempKey as keyof Metric],
      );
      const externalTemp = data.map(
        (item: Metric) => item[externalTempKey as keyof Metric],
      );
      ...
      options: {
        ...
        yaxis: prevChartData.options.yaxis.map((axis: any) => ({
          ...
          labels: {
            ...
            formatter: function (val: any) {
              if (axis.title.text === "Temperatura Interna (°C)") {
                return `${val} °C`;
              } else if (axis.title.text === "Temperatura Externa (°C)") {
                return `${val} °C`;
              }
              return val;
            },
