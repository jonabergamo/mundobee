...
export class MetricsController {
  ...
  @Get("device/:deviceId")
  async findAllByDeviceId(@Param("deviceId") deviceId: string, @Query("from") from?: string, @Query("to") to?: string) {
    const fromDate = from ? new Date(from) : undefined;
    const toDate = to ? new Date(to) : undefined;
    const metrics = await this.metricsService.findAllByDeviceId(deviceId, fromDate, toDate);
    return metrics;
  }
